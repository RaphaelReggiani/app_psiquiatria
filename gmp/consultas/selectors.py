...
def consultas_do_paciente(paciente) -> QuerySet:
    return (
        AgendamentoConsulta.objects.select_related("medico")
...
def historico_paciente_realizadas(paciente_id) -> QuerySet:
    return (
        AgendamentoConsulta.objects.select_related("medico")
        .filter(paciente_id=paciente_id, status=AgendamentoConsulta.STATUS_REALIZADA)
        .order_by("-data_hora")
    )
...
def consultas_realizadas_por_medico(medico) -> QuerySet:
    return (
        ...
        .filter(medico_id=medico.id, status=AgendamentoConsulta.STATUS_REALIZADA)
        .order_by("-data_hora")
    )
...
def agenda_medico_com_filtros(
    ...
    consultas = AgendamentoConsulta.objects.select_related("paciente", "medico").only(
        "id",
        "data_hora",
        "status",
        "paciente__id",
...
def consulta_por_id(consulta_id) -> QuerySet:
    return AgendamentoConsulta.objects.select_related(
        "paciente", "medico", "consulta"
    ).filter(id=consulta_id)


def consulta_marcada_por_id(consulta_id) -> QuerySet:
    return AgendamentoConsulta.objects.select_related("paciente", "medico").filter(
        id=consulta_id, status=AgendamentoConsulta.STATUS_MARCADA
    )
